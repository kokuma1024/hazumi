import type { ConversationMessage, ProResult, VisionResult, VisionProfile } from "../types";

const API_ENDPOINT = "/api/claude";

interface ClaudeApiPayload {
  model: string;
  max_tokens: number;
  system: string;
...
async function callClaudeApi(payload: ClaudeApiPayload): Promise<{ content: Array<{ text?: string }> }> {
  let res: Response;
  try {
...
export async function callProMode(userText: string, conversationHistory: ConversationMessage[]): Promise<ProResult> {
  const pastRoles = conversationHistory
    .filter(m => m.role === "assistant")
    .flatMap(m => { try { const p = JSON.parse(m.content); if (Array.isArray(p)) return p.map((c: { role: string }) => c.role); if (p.tasks && Array.isArray(p.tasks)) return p.tasks.map((c: { role: string }) => c.role); return []; } catch { return []; } });
  const mentorCount = pastRoles.filter((r: string) => r === "mentor").length;
...
export async function callVisionMode(userText: string, profile: VisionProfile, conversationHistory: ConversationMessage[]): Promise<VisionResult> {
...
- tipsはこのタスクをうまく進めるためのコツ・注意点・効率化のヒントを1〜2文で記載`;
  ...
  const text = (data.content || []).map(b => b.text || "").join("").replace(/```json|```/g, "").trim();
  try { return JSON.parse(text) as VisionResult; }
