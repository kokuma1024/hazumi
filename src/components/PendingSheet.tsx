...
interface Props {
  ...
  onEditAndRetry: (item: PendingItem, newText: string) => void;
  onDelete: (id: string) => void;
  onClose: () => void;
}

export function PendingSheet({ items, onResume, onEditAndRetry, onDelete, onClose }: Props) {
        ...
        {confirmDeleteId && (() => {
          ...
          return (
            <div style={{ background: "#fff5f5", border: "1px solid #fca5a5", borderRadius: 14, padding: "14px 16px", marginBottom: 12 }}>
              ...
              <div style={{ fontSize: 12, color: "#64748b", marginBottom: 12, lineHeight: 1.5 }}>{target?.action}</div>
              <div style={{ display: "flex", gap: 8 }}>
                <button style={{ ...S.primaryBtn, background: "#ef4444", flex: 2 }} onClick={() => { onDelete(confirmDeleteId); setConfirmDeleteId(null); }}>削除する</button>
            ...
            <div key={item.id} style={S.pendCard}>
              ...
              {editId === item.id ? (
                <>
                  ...
                  <div style={S.btnRow}>
                    <button style={{ ...S.primaryBtn, background: accent, flex: 2 }} onClick={() => { onEditAndRetry(item, editTxt.trim()); setEditId(null); }}>保存して再提案</button>
