...
export function ContextBubble({ msg }: Props) {
  ...
  return (
    <div style={S.contextBubble}>
      ...
      {msg.userText && <div style={S.contextPrompt}>💬 {msg.userText}</div>}
        ...
        <div style={S.contextHistory}>
          {history.map((h, i) => (
            <div key={i} style={S.contextRow}>
              ...
              <span style={{ ...S.contextRowText, color: h.role === "user" ? "#334155" : "#64748b" }}>{h.text}</span>
            </div>
          ))}
        </div>
      )}
